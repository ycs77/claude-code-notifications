...
description: 互動式建立 Claude Code 多平台提示音插件。
allowed-tools: Bash, Read, Edit
---

# 建立 Claude Code 提示音插件

本 skill 維護專案根目錄的 `notifications.json` 註冊表，並透過 `node build.js` 全量重生 Claude Code plugin 產物。

## 專案規則

- 以 `notifications.json` 作為唯一真相來源。
- 不手動編輯 `plugins/*/`、`.claude-plugin/marketplace.json`，以及根 `README.md` 的插件列表表格；這些內容會由 `build.js` 覆蓋。
